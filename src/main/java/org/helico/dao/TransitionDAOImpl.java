...
public class TransitionDAOImpl implements TransitionDAO {
    ...
    public String getHandlerName(String event, String status) {
    	Transition trans = (Transition)sessionFactory.getCurrentSession().createQuery("from Transition where event=?1 and sourceStatus=?1")
	    .setParameter(1, event).setParameter(1, status).uniqueResult();
